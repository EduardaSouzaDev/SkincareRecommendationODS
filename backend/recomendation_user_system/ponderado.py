...
def recommend(username, users_ratings, game_target=None, k=3):
    ...
    for sim, neighbor in top_neighbors:
        ...
        neighbor_ratings = users_ratings[users_ratings['Username'] == neighbor].set_index('Game')['Rating'].to_dict()
    ...
    if game_target:
        if game_target in game_scores:
            st.write(f"Nota projetada para {username} no jogo '{game_target}': {game_scores[game_target]:.3f}")
        else:
            st.write(f"Não há dados suficientes dos vizinhos para projetar nota para '{game_target}'.")
...
def recommend_app():
    ...
    if not users_ratings.empty:
        ...
        if st.button("Recomendar Jogos"):
            st.write(f"Gerando recomendações para {username} usando kNN com influência ponderada...")
